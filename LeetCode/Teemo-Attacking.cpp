...
+ Since the poisoned status won't add up together, though the second poisoning attack will still work at time point 2, it will stop at
the end of time point 3.
+ So you finally need to output 3. */

int findPoisonedDuration(vector<int>& timeSeries, int duration) {
  int tp = 0, pe = 0;
  for(int i = 0; i < timeSeries.size(); ++i){
      tp += duration - (timeSeries[i] < pe) *(pe - timeSeries[i]);
